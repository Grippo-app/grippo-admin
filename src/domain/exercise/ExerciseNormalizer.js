import {DEFAULT_LANGUAGE, FIELD, SUPPORTED_LANGUAGES} from '../../shared/constants/index.js';
...
export class ExerciseNormalizer {
    static createEmptyTranslations() {
        return createEmptyTranslations();
    }

    static ensureTranslationMap(raw) {
        return ensureTranslationMap(raw);
    }

    static getTranslation(map, locale) {
        return getTranslation(map, locale);
    }

    static cloneArrayOfObjects(arr) {
        return cloneArrayOfObjects(arr);
    }

    static buildLocalizedEntries(map) {
        return buildLocalizedEntries(map);
    }
    ...
    static normalizeEntityShape(raw, opts) {
        return normalizeEntityShape(raw, opts);
    }

    static mergeLocalizedEntity(canonical, incoming) {
        return mergeLocalizedEntity(canonical, incoming);
    }

    static buildPersistencePayload(canonical) {
        return buildPersistencePayload(canonical);
    }

    static normalizeComponents(raw) {
        return normalizeComponents(raw);
    }

    static buildLocalePlaceholder(basePlaceholder, fallback, loc) {
        return buildLocalePlaceholder(basePlaceholder, fallback, loc);
    }
}
