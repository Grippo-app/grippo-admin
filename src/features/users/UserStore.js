...
export class UserStore {
  ...
  setUsers(users)          { this._update({ users, filtered: this._applyFilter(users, this._state.searchQuery) }); }
  setActive(user)          { this._update({ active: user }); }
  clearActive()            { this._update({ active: null }); }
  setSearchQuery(q)        { this._update({ searchQuery: q, filtered: this._applyFilter(this._state.users, q) }); }
  setSortKey(key)          { this._update({ sortKey: key, filtered: this._sortItems([...this._state.filtered], key) }); }
  setLoading(v)            { this._update({ isLoading: v }); }
  setRoleChangeInFlight(v) { this._update({ roleChangeInFlight: v }); }
  ...
  _update(patch) {
    ...
    for (const fn of this._observers) try { fn(this._state); } catch (e) { console.error(e); }
  }
